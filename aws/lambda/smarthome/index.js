...
exports.handler = async function (event, context) {
    ...
    if (namespace.toLowerCase() === 'alexa.discovery') {
        ...
        adr.addPayloadEndpoint({"friendlyName": "Ventilatore", "endpointId": "fan-01", "capabilities": [capability_alexa, capability_alexa_powercontroller], "manufacturerName": "Alexa Academy", "description": "Ventilatore da Arduino", "displayCategories":["FAN"]});
    ...
    if (namespace.toLowerCase() === 'alexa.powercontroller') {
        ...
        let state_set = await sendDeviceState(endpoint_id, "powerState", power_state_value);
        if (!state_set) {
...
async function sendDeviceState(endpoint_id, state, value) {
    ...
    try {
        let request = await dynamodb.updateItem({
            TableName: "FanSkillTable",
